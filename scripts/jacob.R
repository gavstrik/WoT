...
nrow(hmm.dat)
M = get_hmm(hmm.dat, nstates, optim_state = TRUE, use_info = FALSE)
...
layout(1:3, heights = c(3,1,1))
    ...
    # Plot the observed log relative errors
      plot(hmm.dat$X, col=add.alpha('black',.5), type='n', lty=3, ylim=c(-2,4))
      points(hmm.dat$X, col=cols, pch=pchs, cex=2)
      abline(h=0, lty=3)
    
    # Plot the estimated states
      plot(map_states(M$states$state,order(M$mu)), col=add.alpha('black',.5), type='n', lty=3, yaxt='n', cex=2)
    ...
    # Plot the data distribution with normal components and weighted average 
      hist(hmm.dat$X, breaks=50, prob=TRUE, border=NA, col=add.alpha(par()$fg,.25), xlim=c(-4,4), ann=FALSE)
